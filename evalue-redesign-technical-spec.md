...
**Categories (prototype v2.7):** All · **Agile Team · Custom Dev · AI Automation · Web Design** (role-based, added v2.7) · Fintech · E-commerce · SaaS · AI · IoT · Healthcare · Manufacturing
**Filter logic (v2.7):** `filtered()` function now matches on both `c.tag` (industry) AND `c.role` (service type). Previously only matched `c.tag`, so role-based filter buttons returned empty results.
**Categories (production target):** Vše / FinTech / E-commerce / SaaS / Energetika / Nearshore
**`showCasesFiltered(filter)` (v2.7):** Global function that navigates to the Cases page with a pre-set filter active. Called by all service page "All cases" buttons. Sets `currentFilter`, calls `showPage('cases')`, then `renderGrid()` + `syncFilterUI()` via the `showPage` wrapper. Each service page maps to its own filter:

| Service page | Filter applied |
|---|---|
| Agile Team | `'Agile Team'` |
| Body Shopping | `'Agile Team'` (no Body Shopping role in data — closest match) |
| Custom App | `'Custom Dev'` |
| Websites | `'Web Design'` |
...
**Location:** Contact page (alongside the form) · **All 5 service pages** — Agile Team · Body Shopping · Custom App · Websites · AI Automation (v2.6). Each service page has a `mid-cta__box` section above the portfolio carousel containing a primary CTA button + Jan card, with copy tailored to that service.
**Content:** Jan Procházka avatar image · name · role (BDM) · email · "Usually replies in 40m"
**CSS pattern:** `.mid-cta__cta-group` (flex column, items centred) wraps the primary button and `.mid-cta__bdm` card. This ensures button is always above Jan card, never side-by-side.
...
**Click wave effect (v2.7):** Clicking anywhere on the page triggers a 3-ring expanding water ripple animation at the exact click position. Three concentric `.click-wave` divs fire with staggered delays (0 / 120 / 260ms): a small sharp inner ring, a wider purple mid ring, and a large outer ring. Each has a glowing border + radial fill, animates via `transform:scale` + `opacity` only (GPU-composited, zero layout cost), and self-removes from the DOM on `animationend`. Total duration 1.4s. Works in dark and light mode via `data-theme` attribute check. Implemented as a global `document.addEventListener('click')` — no per-element wiring needed.
...
### v2.7 (June 2026) — Click wave, filtered case navigation, button label unification

**Click wave effect:**
- Clicking anywhere on the page spawns 3 concentric expanding rings at the exact click point (`.click-wave`). Rings stagger at 0 / 120 / 260ms with sizes 120 / 240 / 400px. Inner ring is sharp accent blue, mid ring is purple, outer ring is a translucent blue border. Animation duration 1.4s using `cubic-bezier(.1,.7,.2,1)`. Only `transform` + `opacity` animated — GPU-composited, zero layout cost. DOM element self-removes on `animationend`. Adapts to dark/light theme.
- Previous button-only ripple approach (`btn::after` pseudo-element on `:active`) removed — it reset immediately on mouse-up and was invisible on tap.

**Filtered case studies navigation:**
- `showCasesFiltered(filter)` global function added. Navigates to Cases page with a pre-set filter applied and the correct filter button highlighted.
- `filtered()` in the Cases IIFE updated to match `c.tag === filter || c.role === filter` — previously only matched `c.tag`, making role-based filters return zero results.
- `syncFilterUI()` helper added — sets `.active` on the correct filter button after programmatic navigation.
- Cases page filter bar expanded with role-based tabs: **Agile Team · Custom Dev · AI Automation · Web Design** (placed at top of filter row, before industry tabs).
- All 5 service page "All cases" / "See portfolio" buttons now call `showCasesFiltered()` with their respective role instead of `showPage('cases')`:
  - Agile Team → `'Agile Team'`; Body Shopping → `'Agile Team'`; Custom App → `'Custom Dev'`; Websites → `'Web Design'`; AI → `'AI Automation'`

**Button label unification:**
- "View all" on homepage case study section renamed to "All cases" — now consistent with all 4 service page carousel buttons.

---

### v2.6 (June 2026) — Portfolio case studies, announcement bar, service page carousels, Jan CTA on all pages

**Real portfolio case studies (3 new):**
- Three real cases from evalue.cz/weby.html added to both `ALL_CASES` JS array (Cases page dynamic grid) and static HTML (Websites page portfolio strip):
  - **lucietomaskova.cz** — psychotherapy website, Web Design / Custom Dev, 48h delivery, SEO optimised
  - **smzahrady.cz** — garden design & landscaping, Web Design / Custom Dev, Local SEO
  - **hasicibohusov.cz** — volunteer fire brigade, Web Design / Non-profit, CMS self-editable
- Case modal `openCaseModal()` updated to show a "Visit website" button (with `open_in_new` icon) when the case data object includes a `url` field. Previously all modals only had "Start a similar project." `url` field added to all 3 real portfolio cases.

**Sitewide announcement bar:**
- `#announceBar` added above `<nav>` (outside all `.page` divs — always visible). Amber gradient (`#92400e → #d97706`), dismissible via ✕ button (sessionStorage). Clicking the bar navigates to the Summer Deal page.
- Auto-hidden when the Summer Deal page is active (avoids redundant promotion on the promotion page itself).
- `showPage()` wrapper updated: bar hides when `id === 'summer'` or when user has dismissed it.

**Special Offers nav:**
- "Summer Deal" nav link renamed to **"Special Offers"** (still routes to `page-summer`). Rationale: "Special Offers" is a permanent label that works for any seasonal promotion; "Summer Deal" would be stale in autumn.
- Nav link styled as a standard `.nav__link` — removed the palm tree emoji + orange colour treatment that made it visually inconsistent with the rest of the nav.

**Websites page — real data:**
- Page translated fully to English.
- Pricing updated to real data: Presentational Website **2,990 CZK one-off**, 48h delivery. Business Website from 8,990 CZK. E-shop / Portal from 25,000 CZK.
- Portfolio strip added with the 3 real case study cards (lucietomaskova.cz / smzahrady.cz / hasicibohusov.cz).

**Related cases carousels on service pages:**
- Horizontal carousel (`svc-cases` / `svc-cases__track`) added to all 5 service pages (Agile Team, Body Shopping, Custom App, Websites, AI Automation), each with 3 relevant case cards, prev/next nav buttons, and an "All cases" button.
- `svcScroll(trackId, dir)` JS function handles carousel scrolling via `translateX`.
- **Architecture note:** All carousel `<section>` elements must be inside their respective `.page` div before its closing `</div>`. Elements outside a `.page` div are permanently visible regardless of current page. Early implementation placed carousels outside page divs — fixed by moving them inside.

**Jan Procházka CTA section — all service pages:**
- Websites and AI Automation pages were missing the Jan Procházka CTA block that Agile Team, Body Shopping, and Custom App already had. Added to both — placed between the main content section and the portfolio/cases carousel. Copy tailored per page:
  - Websites: "Ready to launch your website?" + "Order a website →"
  - AI: "Curious what AI could automate for you?" + "Book AI discovery call →"

**Bug fixes:**
- Cursor glow (`#cursor-glow`): opacity increased from 0.06/0.04 (invisible) to 0.18/0.14. Positioning switched from `transform:translate` to `left`/`top` properties (required for GPU compositing of transition).
- Case card layout: `.case-card__read-more` pinned to bottom of card body with `margin-top:auto`. Prevents "Read more" button jumping up when card content is short.
- Jan Procházka + submit button alignment: unified via `.mid-cta__cta-group` (flex column, `align-items:center`) across all service pages. Removed per-instance inline flex-row styles that caused Jan card to sit beside the button instead of below it.
